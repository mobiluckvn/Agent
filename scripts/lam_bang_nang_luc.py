...
NEN: list[tuple[str, str, str, str, str, str, str]] = [
    ...
    ("C2.3", "2. Kiểm tra công cụ hiện có", "Dò môi trường: kiến trúc CPU, quyền root, mạng ra ngoài, domain bị chặn", DU,
     ...
     "", KHONG),
    ("C2.4", "2. Kiểm tra công cụ hiện có", "Dò runtime: python --version, pip list, npm ls, which, docker ps", DU,
     "eaa environ --packages liệt kê gói Python/npm đã cài — 'máy này sẵn có gì', khác với 'công cụ trong Tool Card đã cài chưa' của doctor · TC-77",
     "", KHONG),
    ...
    ("C2.8", "2. Kiểm tra công cụ hiện có", "Kiểm phiên bản / tương thích", DU,
     ...
     "", KHONG),
    ("C2.9", "2. Kiểm tra công cụ hiện có", "Phát hiện xung đột phụ thuộc", DU,
     "eaa/installplan.py find_conflicts(): hai thẻ cùng đòi một thứ ở hai ràng buộc loại trừ nhau thì CHẶN cả kế hoạch cài. Chỉ báo khi CHẮC CHẮN đá nhau · eaa doctor --plan · TC-76",
     "", KHONG),
    ...
    # ---------------------------------------------------------------- C4 ---
    ("C4.1", "4. Cài đặt công cụ", "Chọn cơ chế cài phù hợp", DU,
     "Tool Card khai method: gói / nhị phân / nguồn / container / môi trường riêng; mỗi cách kèm lời giải thích nó hỏng kiểu gì. Nhị phân thiếu checksum thì cảnh báo · TC-76",
     "", KHONG),
    ("C4.2", "4. Cài đặt công cụ", "Cô lập môi trường (venv riêng cho tool mới)", DU,
     "method 'môi trường riêng' cho công cụ ngoài; cổng chạy thử của xưởng công cụ vốn đã chạy trong thư mục tạm riêng, không mạng, không khoá API",
     "", KHONG),
    ("C4.3", "4. Cài đặt công cụ", "Cài theo thứ tự phụ thuộc (system lib trước, runtime package sau)", DU,
     "Tool Card khai requires; plan_installs() sắp xếp tô-pô ỔN ĐỊNH, bắt phụ thuộc vòng, và NÊU RA phụ thuộc trỏ ngoài manifest thay vì nuốt · TC-76",
     "", KHONG),
    ...
    ("C6.7", "6. Tự viết code tạo công cụ mới", "Kiểm bảo mật mã tự sinh: không hard-code secret, không eval input", DU,
     ...
     "", KHONG),
    ("C6.8", "6. Tự viết code tạo công cụ mới", "Tài liệu hoá tool mới: README ngắn, ví dụ, giới hạn đã biết", DU,
     "eaa tool doc sinh tài liệu TỪ MÃ (MO_TA, SCHEMA, hàm test_) cộng số đo dùng thật — không hỏi mô hình, vì tài liệu lệch khỏi mã tệ hơn không có tài liệu · TC-77",
     "", KHONG),
    ...
    ("C7.2", "7. Đăng ký & mở rộng năng lực", "Cập nhật bản đồ năng lực để lần sau không tìm lại", DU,
     ...
     "", KHONG),
    ("C7.3", "7. Đăng ký & mở rộng năng lực", "Versioning tool, giữ bản cũ nếu bản mới hỏng", DU,
     "Mỗi lần ghi đè một bản ĐÃ DUYỆT thì cất bản cũ; eaa tool rollback quay về, và bản quay về phải đi lại ba cổng · TC-77",
     "", KHONG),
    ...
    ("C10.2", "10. Không đóng cứng quy trình", "Vào giữa quy trình, bỏ qua giai đoạn không cần", DU,
     ...
     "", KHONG),
    ("C10.3", "10. Không đóng cứng quy trình", "Chế độ không cần toolchain / không cần phần cứng", DU,
     "eaa gen --preview sinh mã rồi DỪNG: không cổng, không nhánh, không commit. An toàn hơn cả chế độ nháp, và nới đúng một tiền điều kiện (pha) · TC-77",
     "", KHONG),
